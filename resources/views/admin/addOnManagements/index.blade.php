...
@can('add_on_management_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.add-on-managements.create') }}">
                        ...
                        <tr data-entry-id="{{ $addOnManagement->id }}">
                            ...
                            <td>
                                @can('add_on_management_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.add-on-managements.show', $addOnManagement->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('add_on_management_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.add-on-managements.edit', $addOnManagement->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('add_on_management_delete')
                                    <form action="{{ route('admin.add-on-managements.destroy', $addOnManagement->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('add_on_management_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.add-on-managements.massDestroy') }}",
