...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.order-managements.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('delivery_charge') ? 'is-invalid' : '' }}" type="number" name="delivery_charge" id="delivery_charge" value="{{ old('delivery_charge', '') }}" step="0.01">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('voucher_used') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="voucher_used" id="voucher_used" value="1" {{ old('voucher_used', 0) == 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('order_type') ? 'is-invalid' : '' }}" name="order_type_id" id="order_type_id">
                    @foreach($order_types as $id => $order_type)
                        <option value="{{ $id }}" {{ old('order_type_id') == $id ? 'selected' : '' }}>{{ $order_type }}</option>
                    @endforeach
                </select>
