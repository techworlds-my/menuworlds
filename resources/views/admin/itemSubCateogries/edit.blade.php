...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.item-sub-cateogries.update", [$itemSubCateogry->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="title">{{ trans('cruds.itemSubCateogry.fields.title') }}</label>
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $itemSubCateogry->title) }}" required>
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('is_enable') ? 'is-invalid' : '' }}" type="text" name="is_enable" id="is_enable" value="{{ old('is_enable', $itemSubCateogry->is_enable) }}">
