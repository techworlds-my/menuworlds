...
@can('seats_management_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.seats-managements.create') }}">
                        ...
                        <tr data-entry-id="{{ $seatsManagement->id }}">
                            ...
                            <td>
                                @can('seats_management_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.seats-managements.show', $seatsManagement->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('seats_management_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.seats-managements.edit', $seatsManagement->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('seats_management_delete')
                                    <form action="{{ route('admin.seats-managements.destroy', $seatsManagement->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('seats_management_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.seats-managements.massDestroy') }}",
