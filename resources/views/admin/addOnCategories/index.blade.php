...
@can('add_on_category_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.add-on-categories.create') }}">
                        ...
                        <tr data-entry-id="{{ $addOnCategory->id }}">
                            ...
                            <td>
                                @can('add_on_category_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.add-on-categories.show', $addOnCategory->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('add_on_category_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.add-on-categories.edit', $addOnCategory->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('add_on_category_delete')
                                    <form action="{{ route('admin.add-on-categories.destroy', $addOnCategory->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('add_on_category_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.add-on-categories.massDestroy') }}",
