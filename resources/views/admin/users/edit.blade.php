...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.users.update", [$user->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_merchant') ? 'is-invalid' : '' }}">
                    <input type="hidden" name="is_merchant" value="0">
                    <input class="form-check-input" type="checkbox" name="is_merchant" id="is_merchant" value="1" {{ $user->is_merchant || old('is_merchant', 0) === 1 ? 'checked' : '' }}>
                    <label class="form-check-label" for="is_merchant">{{ trans('cruds.user.fields.is_merchant') }}</label>
                </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('point') ? 'is-invalid' : '' }}" type="number" name="point" id="point" value="{{ old('point', $user->point) }}" step="1">
                @if($errors->has('point'))
                    <div class="invalid-feedback">
                        {{ $errors->first('point') }}
                    </div>
