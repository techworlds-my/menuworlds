...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.item-catrgories.update", [$itemCatrgory->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="title">{{ trans('cruds.itemCatrgory.fields.title') }}</label>
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $itemCatrgory->title) }}" required>
                @if($errors->has('title'))
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_enable') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="is_enable" id="is_enable" value="1" {{ $itemCatrgory->is_enable || old('is_enable', 0) === 1 ? 'checked' : '' }}>
