    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.voucher-wallets.update", [$voucherWallet->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="username_id" id="username_id">
                                @foreach($usernames as $id => $username)
                                    <option value="{{ $id }}" {{ (old('username_id') ? old('username_id') : $voucherWallet->username->id ?? '') == $id ? 'selected' : '' }}>{{ $username }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            <div>
                                ...
                                <input type="checkbox" name="is_redeem" id="is_redeem" value="1" {{ $voucherWallet->is_redeem || old('is_redeem', 0) === 1 ? 'checked' : '' }}>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="voucher_id" id="voucher_id">
                                @foreach($vouchers as $id => $voucher)
                                    <option value="{{ $id }}" {{ (old('voucher_id') ? old('voucher_id') : $voucherWallet->voucher->id ?? '') == $id ? 'selected' : '' }}>{{ $voucher }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="usage" id="usage" value="{{ old('usage', $voucherWallet->usage) }}">
                            @if($errors->has('usage'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('usage') }}
                                </div>
