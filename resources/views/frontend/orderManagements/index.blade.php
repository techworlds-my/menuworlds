        ...
        <div class="col-md-12">
            @can('order_management_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.order-managements.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $orderManagement->id }}">
                                        ...
                                        <td>
                                            @can('order_management_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.order-managements.show', $orderManagement->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('order_management_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.order-managements.edit', $orderManagement->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('order_management_delete')
                                                <form action="{{ route('frontend.order-managements.destroy', $orderManagement->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('order_management_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.order-managements.massDestroy') }}",
