        ...
        <div class="col-md-12">
            @can('item_category_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.item-categories.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $itemCategory->id }}">
                                        ...
                                        <td>
                                            @can('item_category_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.item-categories.show', $itemCategory->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('item_category_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.item-categories.edit', $itemCategory->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('item_category_delete')
                                                <form action="{{ route('frontend.item-categories.destroy', $itemCategory->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('item_category_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.item-categories.massDestroy') }}",
