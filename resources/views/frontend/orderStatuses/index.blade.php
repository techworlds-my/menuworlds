        ...
        <div class="col-md-12">
            @can('order_status_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.order-statuses.create') }}">
                            ...
                            <tbody>
                                @foreach($orderStatuses as $key => $orderStatus)
                                    <tr data-entry-id="{{ $orderStatus->id }}">
                                        ...
                                        <td>
                                            <span style="display:none">{{ $orderStatus->in_enable ?? '' }}</span>
                                            <input type="checkbox" disabled="disabled" {{ $orderStatus->in_enable ? 'checked' : '' }}>
                                        </td>
                                        <td>
                                            @can('order_status_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.order-statuses.show', $orderStatus->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('order_status_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.order-statuses.edit', $orderStatus->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('order_status_delete')
                                                <form action="{{ route('frontend.order-statuses.destroy', $orderStatus->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('order_status_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.order-statuses.massDestroy') }}",
