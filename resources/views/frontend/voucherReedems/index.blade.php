        ...
        <div class="col-md-12">
            @can('voucher_reedem_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.voucher-reedems.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $voucherReedem->id }}">
                                        ...
                                        <td>
                                            @can('voucher_reedem_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.voucher-reedems.show', $voucherReedem->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('voucher_reedem_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.voucher-reedems.edit', $voucherReedem->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('voucher_reedem_delete')
                                                <form action="{{ route('frontend.voucher-reedems.destroy', $voucherReedem->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('voucher_reedem_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.voucher-reedems.massDestroy') }}",
