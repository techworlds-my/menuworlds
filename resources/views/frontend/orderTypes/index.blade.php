        ...
        <div class="col-md-12">
            @can('order_type_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.order-types.create') }}">
                            ...
                            <tbody>
                                @foreach($orderTypes as $key => $orderType)
                                    <tr data-entry-id="{{ $orderType->id }}">
                                        ...
                                        <td>
                                            @can('order_type_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.order-types.show', $orderType->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('order_type_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.order-types.edit', $orderType->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('order_type_delete')
                                                <form action="{{ route('frontend.order-types.destroy', $orderType->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('order_type_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.order-types.massDestroy') }}",
