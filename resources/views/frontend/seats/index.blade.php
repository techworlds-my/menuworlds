        ...
        <div class="col-md-12">
            @can('seat_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.seats.create') }}">
                            ...
                            <tbody>
                                @foreach($seats as $key => $seat)
                                    <tr data-entry-id="{{ $seat->id }}">
                                        ...
                                        <td>
                                            <span style="display:none">{{ $seat->used ?? '' }}</span>
                                        ...
                                        <td>
                                            @can('seat_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.seats.show', $seat->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('seat_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.seats.edit', $seat->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('seat_delete')
                                                <form action="{{ route('frontend.seats.destroy', $seat->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('seat_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.seats.massDestroy') }}",
