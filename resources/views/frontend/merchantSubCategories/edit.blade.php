    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.merchant-sub-categories.update", [$merchantSubCategory->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            <label class="required" for="sub_category">{{ trans('cruds.merchantSubCategory.fields.sub_category') }}</label>
                            <input class="form-control" type="text" name="sub_category" id="sub_category" value="{{ old('sub_category', $merchantSubCategory->sub_category) }}" required>
                        ...
                        <div class="form-group">
                            <div>
                                ...
                                <input type="checkbox" name="in_enable" id="in_enable" value="1" {{ $merchantSubCategory->in_enable || old('in_enable', 0) === 1 ? 'checked' : '' }}>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="category_id" id="category_id" required>
                                @foreach($categories as $id => $category)
                                    <option value="{{ $id }}" {{ (old('category_id') ? old('category_id') : $merchantSubCategory->category->id ?? '') == $id ? 'selected' : '' }}>{{ $category }}</option>
                                @endforeach
                            </select>
